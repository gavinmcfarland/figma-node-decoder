	...
	${Ref(node)}.fontName = {
		...
		style: ${JSON.stringify(node.fontName?.style)}
						...
						if (name !== 'width' && name !== 'height' && !textProps.includes(name) && !styleProps.includes(name)) {
							...
							let exceptions = ['fontWeight', 'absoluteBoundingBox', 'stuckNodes', 'componentPropertyReferences', 'canUpgradeToNativeBidiSupport', 'itemReverseZIndex', 'strokesIncludedInLayout', 'attachedConnectors'];
							if (options?.[name] !== false && exceptions.indexOf(name) == -1 && JSON.stringify(value) !== 'undefined') {
	...
	function generateImagesHash(nodes, data) {
		...
		for (let i in nodes) {
			...
			if (typeof nodeElement != 'undefined') {
				...
				if (nodeElement.type == 'FRAME' || nodeElement.type == 'INSTANCE' || nodeElement.type == 'COMPONENT' || nodeElement.type == 'IMAGE' || nodeElement.type == 'RECTANGLE') {
