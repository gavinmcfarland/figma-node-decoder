...
import Str from "./str";
import {
	getInstanceCounterpart,
	getTopInstance,
	getInstanceCounterpartUsingLocation,
	getOverrides,
	getNodeDepth,
	getParentInstance,
	getNoneGroupParent,
...
export async function genPluginStr(origSel, opts?) {
	...
	function StyleRef(style) {
		return (
			v.lowerCase(style.name.replace(/\s|\//g, "_").replace(/\./g, "")) +
			"_" +
			style.key.slice(-4)
		);
	}
			...
			if (componentSet) {
				if (componentSet.type === "COMPONENT_SET") {
					if (
						...
						if (
							defaultVariant &&
							defaultVariant.id !== node.mainComponent.id
						) {
							...
							if (
								...
								if (shouldResizeHeight || shouldResizeWidth) {
									...
									if (
										...
										string += `	${Ref(
											node
										)}.primaryAxisSizingMode = ${JSON.stringify(
											node.primaryAxisSizingMode
										)}\n`;
									}
									...
									if (
										...
										string += `	${Ref(
											node
										)}.counterAxisSizingMode = ${JSON.stringify(
											node.counterAxisSizingMode
										)}\n`;
									}
								}
...
${fontsString}
	...
	function appendNode(node) {
		...
		if (node.parent) {
			if (
				...
				str`	${Ref(getNoneGroupParent(node))}.appendChild(${Ref(
					node
				)})\n`;
	...
	function createBasic(node, level, options = {}) {
		if (node.type === "COMPONENT") {
			...
			if (
				allComponents.some(
					(component) =>
						JSON.stringify(component) === JSON.stringify(node)
				)
			) {
				return true;
			}
	...
	function createBooleanOperation(node, level) {
		...
		if (node.type === "BOOLEAN_OPERATION" && !isInsideInstance(node)) {
			var children: any = Ref(node.children);
			if (Array.isArray(children)) {
				children = Ref(node.children).join(", ");
			}
	...
	var ${Ref(node)} = figma.${v.lowerCase(
